  ...
  if ( ! mfrc522.PICC_ReadCardSerial()) {//if PICC_ReadCardSerial returns 1, the "uid" struct (see MFRC522.h lines 238-45)) contains the ID of the read card.
    ...
    //byte    sak;      // The SAK (Select acknowledge) byte returned from the PICC after successful selection.
       ...
       //mfrc522.PICC_DumpToSerial(&(mfrc522.uid));//uncomment this if you want to see the entire 1k memory with the block written into it.
         ...
         Serial.println("");
          readBlock(13, readbackblock);//read the block back
         Serial.print("AGE: ");
         for (int j=0 ; j<16 ; j++)//print the block contents
         {if(readbackblock[j]!=a)
           Serial.write (readbackblock[j]);//Serial.write() transmits the ASCII numbers as human readable characters to serial monitor
        ...
        readBlock(4, readbackblock);//read the block back
         ...
         Serial.println("");
          readBlock(12, readbackblock);//read the block back
         Serial.print("Medicine 2: ");
         for (int j=0 ; j<16 ; j++)//print the block contents
         {if(readbackblock[j]!=a)
           Serial.write (readbackblock[j]);//Serial.write() transmits the ASCII numbers as human readable characters to serial monitor
         }
         ...
         Serial.println("");
          readBlock(9, readbackblock);//read the block back
         Serial.print("Medicine 3: ");
         for (int j=0 ; j<16 ; j++)//print the block contents
         {
          if(readbackblock[j]!=a)
             Serial.write (readbackblock[j]);//Serial.write() transmits the ASCII numbers as human readable characters to serial monitor
         }
         readBlock(8, readbackblock);//read the block back
